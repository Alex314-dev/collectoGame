 ...
 * <p>This class does not use optimal Java programming practices.
 * It is designed specifically to be easily
 * usable even by a beginning programmer who has not yet learned about objects and exceptions.
 * Therefore, 
 * everything is in a single source file that compiles into a single class file, all the methods are
 * static methods,
 * and none of the methods throw exceptions that would require try...catch statements.
...
public class TextIO {
    ...
    public static void readFile(String fileName) {
        if (fileName == null) {
        	// Go back to reading standard input
            readStandardInput();
        } else {
     ...
     * (specified by the constant TextIO.EOF).An end-of-line is always returned
     * as the character '\n', even when the actual
     * end-of-line in the input source is something else,
     ...
     * the next input character is either an
     * end-of-line, an end-of-file, or a non-whitespace character.
     * This method never causes an error.
     * (Ordinarily, end-of-file is not possible when reading from
     * standard input.)
     */
    ...
    /**
     * Skips whitespace characters and then reads
     * a value of type byte from input, discarding the rest of
     * the current line of input (including the
     * next end-of-line character, if any).  When using standard IO,
     * this will not produce an error; the user
     * will be prompted repeatedly for input until a legal value
     * is input.  In other cases, an IllegalArgumentException
    ...
    /**
     * Skips whitespace characters and then reads a
     * value of type short from input, discarding the rest of
     * the current line of input (including the next
     * end-of-line character, if any).  When using standard IO,
     * this will not produce an error; the user will
     * be prompted repeatedly for input until a legal value
     * is input.  In other cases, an IllegalArgumentException
     * will be thrown if a legal value is not found.
     */
    ...
    /**
     * Skips whitespace characters and then reads a
     * value of type int from input, discarding the rest of
     * the current line of input (including the next
     * end-of-line character, if any).  When using standard IO,
     * this will not produce an error; the user will be
     * prompted repeatedly for input until a legal value
     * is input.  In other cases, an IllegalArgumentException
     * will be thrown if a legal value is not found.
     */
    ...
    /**
     * Skips whitespace characters and then reads a
     * value of type long from input, discarding the rest of
     * the current line of input (including the next
     * end-of-line character, if any).  When using standard IO,
     * this will not produce an error; the user will
     * be prompted repeatedly for input until a legal value
    ...
    /**
     * Skips whitespace characters and then reads a
     * value of type float from input, discarding the rest of
     * the current line of input (including the next
     * end-of-line character, if any).  When using standard IO,
     * this will not produce an error; the user will be
     * prompted repeatedly for input until a legal value
    ...
    /**
     * Skips whitespace characters and then reads a
     * value of type double from input, discarding the rest of
     * the current line of input (including the next
     * end-of-line character, if any).  When using standard IO,
     * this will not produce an error; the user will be
     * prompted repeatedly for input until a legal value
    ...
    /**
     * Skips whitespace characters and then reads a
     * value of type char from input, discarding the rest of
     * the current line of input (including the next
     * end-of-line character, if any).  Note that the value
     * that is returned will be a non-whitespace character;
     * compare this with the getAnyChar() method.
     * When using standard IO, this will not produce an error.
     * In other cases, an error can occur if
    ...
    /**
     * Skips whitespace characters and then reads a
     * value of type boolean from input, discarding the rest of
     * the current line of input (including the next
     * end-of-line character, if any).  When using standard IO,
     * this will not produce an error; the user will be
     * prompted repeatedly for input until a legal value
     * is input.  In other cases, an IllegalArgumentException
     * will be thrown if a legal value is not found.
     * <p>Legal inputs for a boolean input are: true, t, yes,
     * y, 1, false, f, no, n, and 0; letters can be
     * either upper case or lower case. One "word" of input is read,
     * using the getWord() method, and it
     * must be one of these; note that the "word"  must be
     * terminated by a whitespace character (or end-of-file).
     */
     ...
     * (including the next end-of-line character, if any).  A word is defined as
    ...
    /**
     * Reads all the characters from the current input source,
     * up to the next end-of-line.  The end-of-line
     * is read but is not included in the return value.
     * Any other whitespace characters on the line are retained,
     * even if they occur at the start of input.
     * The return value will be an empty string if there are
     * no characters before the end-of-line.
     * When using standard IO, this will not produce an error.
    ...
    /**
     * Skips whitespace characters and then reads a
     * value of type byte from input.  Any additional characters on
     * the current line of input are retained, and will be
     * read by the next input operation.  When using standard IO,
     * this will not produce an error; the user will be
     * prompted repeatedly for input until a legal value
     * is input.  In other cases, an IllegalArgumentException
     * will be thrown if a legal value is not found.
    ...
    /**
     * Skips whitespace characters and then reads
     * a value of type short from input.  Any additional characters on
     * the current line of input are retained, and will
     * be read by the next input operation.  When using standard IO,
     * this will not produce an error; the user will be
     * prompted repeatedly for input until a legal value
     * is input.  In other cases, an IllegalArgumentException
     * will be thrown if a legal value is not found.
    ...
    /**
     * Skips whitespace characters and then reads
     * a value of type int from input.  Any additional characters on
     * the current line of input are retained, and will
     * be read by the next input operation.  When using standard IO,
     * this will not produce an error; the user will be
     * prompted repeatedly for input until a legal value
    ...
    /**
     * Skips whitespace characters and then reads a
     * value of type long from input.  Any additional characters on
     * the current line of input are retained, and will
     * be read by the next input operation.  When using standard IO,
     * this will not produce an error; the user will be
     * prompted repeatedly for input until a legal value
    ...
    /**
     * Skips whitespace characters and then reads a single
     * non-whitespace character from input.  Any additional characters on
     * the current line of input are retained, and will be
     * read by the next input operation.  When using standard IO,
     * this will not produce an error.  In other cases,
     * an IllegalArgumentException will be thrown if an end-of-file
     * is encountered.
     */
    ...
    /**
     * Skips whitespace characters and then reads a
     * value of type float from input.  Any additional characters on
     * the current line of input are retained, and will
     * be read by the next input operation.  When using standard IO,
     * this will not produce an error; the user will be
     * prompted repeatedly for input until a legal value
    ...
    /**
     * Skips whitespace characters and then reads a
     * value of type double from input.  Any additional characters on
     * the current line of input are retained, and will be
     * read by the next input operation.  When using standard IO,
     * this will not produce an error; the user will be prompted
     * repeatedly for input until a legal value
     * is input.  In other cases, an IllegalArgumentException
     * will be thrown if a legal value is not found.
     */
     ...
     * the current line of input are retained, and will be read
     * by the next input operation.  A word is defined as
     * a sequence of non-whitespace characters (not just letters!).   When using standard IO,
     * this will not produce an error.  In other cases, an IllegalArgumentException will be thrown
     * if an end-of-file is encountered.
     */
    ...
    /**
     * Skips whitespace characters and then reads a value
     * of type boolean from input.  Any additional characters on
     * the current line of input are retained, and will be
     * read by the next input operation.  When using standard IO,
     * this will not produce an error; the user will be
     * prompted repeatedly for input until a legal value
     * is input.  In other cases, an IllegalArgumentException
     * will be thrown if a legal value is not found.
     * <p>Legal inputs for a boolean input are: true, t, yes, y, 1,
     * false, f, no, n, and 0; letters can be
     * either upper case or lower case. One "word" of input is read,
     * using the getWord() method, and it
     * must be one of these; note that the "word"
    ...
    // point is private implementation detail *******************

    private static String inputFileName;  
    // Name of file that is the current input source, or null if the source is not a file.
    
    private static String outputFileName; 
    // Name of file that is the current output destination,
    // or null if the destination is not a file.

    private final static BufferedReader STANDARDINPUT 
    	= new BufferedReader(new InputStreamReader(System.in));  // wraps standard input stream
    private final static PrintWriter STANDARDOUTPUT 
    	= new PrintWriter(System.out);  // wraps standard output stream
    ...
    private final static Pattern INTEGERREGEX 
    	= Pattern.compile("(\\+|-)?[0-9]+");
    
    private final static Pattern FLOATREGEX 
    	= Pattern.compile("(\\+|-)?(([0-9]+(\\.[0-9]*)?)|(\\.[0-9]+))((e|E)(\\+|-)?[0-9]+)?");
    ...
    private static String readRealString() {   
    	// read chars from input following syntax of real numbers
        skipWhitespace();
        if (lookChar() == EOF) {
            return null;
        } 
    ...
    private static void errorMessage(String message, String expecting) {  // Report error on input.
        if (readingStandardInput && writingStandardOutput) {
            ...
            if (inputErrorCount >= 10) {
                throw new IllegalArgumentException("Too many input consecutive "
                		+ "input errors on standard input.");
            }
    ...
    private static char readChar() {  // return and discard next character from input
        ...
        if (buffer == null) {
            if (readingStandardInput) {
                throw new IllegalArgumentException("Attempt to read "
                		+ "past end-of-file in standard input???");
            } else {
                throw new IllegalArgumentException("Attempt to read "
                		+ "past end-of-file in file \"" + inputFileName + "\".");
            }
        }
        pos++;
        return ch;
    ...
    private static void fillBuffer() {    // Wait for user to type a line and press return,
        try {
            ...
            if (readingStandardInput) {
                ...
                throw new IllegalArgumentException("Errow while attempting to "
                		+ "read form an input stream.");
            }
        }
        pos = 0;
        floatMatcher = null;
        ...
        if (writingStandardOutput) {
            ...
            if (outputErrorCount >= 10) {
                ...
                throw new IllegalArgumentException("Too many "
                		+ "errors while writing to standard output.");
            }
            ...
            throw new IllegalArgumentException("Error occurred "
            		+ "while writing to output stream:\n   " + message);
        }
    }
